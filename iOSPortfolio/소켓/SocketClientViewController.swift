...
class SocketClientViewController: UIViewController {
    ...
    func readResponse(from client: TCPClient) -> String? {
        //서버와 클라이언트는 받는 부분과 보내는 부분이 스레드로
        //구현되어야 한다.
        //스레드로 구현하지 않은 상태에서 이 방식으로 구현을 하면
        //클라이언트가 전송한 내용은 서버에게 전달되지만
        //서버가 보낸 내용은 클라이언트가 읽을 수가 없다.
        //서버가 보내면 조금 기다렸다가 읽기 위해서 sleep 을
        //설정한 것이다.
        //소켓 통신을 할 때 반드시 알아 두어야 할 것 중의 하나가
        //보내는 부분과 받는 부분이 별도의 스레드로 구현되어야
        //한다는 것이다.
        sleep(3)
