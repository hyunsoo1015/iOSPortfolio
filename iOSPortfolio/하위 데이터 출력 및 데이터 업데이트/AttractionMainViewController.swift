...
class AttractionMainViewController: UITableViewController {
    ...
    @objc func handleRequest(_ refreshControl: UIRefreshControl) {
        ...
        //서버에서 데이터를 매 번 가져오는 부분을 구현할 때는 서버의 데이터
        //생성을 잘 해야 한다.
        var mainVO = MainVo()
        mainVO.title = "그랜드 캐니언"
        ...
        //일정시간 대기 - 원래는 안함
        //네이버나 다음 같은 Open API 의 데이터를 많이 가져올 때는
        //중간에 sleep을 넣기도 한다.
    ...
    override func viewDidLoad() {
        ...
        //운영체제 별로 다르게 적용 - 10.0 이상과 아래버전을 다르게 적용
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell!.imageView!.image = UIImage(named: mainVO.imageName)
    ...
    //서버로부터 데이터를 받아서 사용할 때는 페이지 번호나 현재 데이터
    //개수와 전체 데이터 개수를 파악해서 2개의 데이터 개수가 같으면
    //아무일도 하지 않아야 한다.
